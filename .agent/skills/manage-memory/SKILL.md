...
name: manage-memory
description: >-
  Manage the persistent memory plane, retrieve indexed context packets (<=4000 tokens) before workflow phases, generate session briefs (<=300 tokens), append Architecture Decision Records (ADRs), update context_index.yaml, track token ledger, and leverage semantic cache.
---
